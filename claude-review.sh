...
# Adapted for GrowthNav - Shared Analytics Infrastructure Monorepo
...
usage() {
    ...
    echo "                      security, performance, testing, bigquery, reporting,"
    echo "                      conversions, mcp, architecture, style"
    ...
    echo "  $0 --focus bigquery 54          # Focus on BigQuery client patterns"
    echo "  $0 --focus reporting 54         # Focus on Sheets/Slides/PDF generation"
    echo "  $0 --focus conversions 54       # Focus on conversion tracking"
    echo "  $0 --focus mcp 54               # Focus on MCP server implementation"
...
# Helper function to detect BigQuery files
has_bigquery_files() {
    gh pr diff "$PR_NUM" --name-only | grep -E "(shared-bigquery|bigquery)" > /dev/null 2>&1
}

# Helper function to detect reporting files (Sheets, Slides, PDF)
has_reporting_files() {
    gh pr diff "$PR_NUM" --name-only | grep -E "(shared-reporting|sheets|slides|pdf)" > /dev/null 2>&1
}

# Helper function to detect conversions files
has_conversions_files() {
    gh pr diff "$PR_NUM" --name-only | grep -E "(shared-conversions|conversions|attribution)" > /dev/null 2>&1
}

# Helper function to detect MCP server files
has_mcp_files() {
    gh pr diff "$PR_NUM" --name-only | grep -E "(mcp-server|growthnav_mcp)" > /dev/null 2>&1
}
...
generate_review_prompt() {
    local base_prompt="Please review this pull request for GrowthNav, a Python analytics monorepo, and provide feedback on:
- Code quality and Python best practices
- Potential bugs or issues
- Performance considerations
- Security concerns (especially GCP credentials handling)
- Test coverage
- Type safety and Pydantic model usage
- Adherence to namespace package patterns (growthnav.*)

Be constructive and helpful in your feedback. This is a uv workspace monorepo where packages/ contains shared libraries using the growthnav.* namespace."

    local additional_prompt=""

    # Add BigQuery specific prompts if relevant files are detected
    if has_bigquery_files || [[ "$FOCUS_AREAS" == *"bigquery"* ]]; then
        additional_prompt="${additional_prompt}

For BigQuery code, also review:
- Multi-tenant dataset isolation (growthnav_{customer_id} pattern)
- Customer registry and industry tracking
- Query parameterization to prevent SQL injection
- Proper use of google-cloud-bigquery client
- Cost optimization (query complexity, data scanning)
- Error handling for API limits and quotas"
    fi

    # Add reporting specific prompts if relevant files are detected
    if has_reporting_files || [[ "$FOCUS_AREAS" == *"reporting"* ]]; then
        additional_prompt="${additional_prompt}

For reporting code (Sheets, Slides, PDF), also review:
- Google API authentication patterns (service account, domain-wide delegation)
- Proper use of impersonation (subject parameter) for Workspace APIs
- Rate limiting considerations (60 requests/minute per user)
- File cleanup and resource management
- PDF generation with WeasyPrint/pdfkit
- Sharing and permission handling"
    fi

    # Add conversions specific prompts if relevant files are detected
    if has_conversions_files || [[ "$FOCUS_AREAS" == *"conversions"* ]]; then
        additional_prompt="${additional_prompt}

For conversions code, also review:
- Attribution model correctness
- Data normalization patterns
- Schema validation with Pydantic
- Integration with multiple ad platforms
- Event tracking accuracy
- Cross-device/cross-channel attribution"
    fi

    # Add MCP server specific prompts if relevant files are detected
    if has_mcp_files || [[ "$FOCUS_AREAS" == *"mcp"* ]]; then
        additional_prompt="${additional_prompt}

For MCP server code, also review:
- Tool definitions and parameter schemas
- Resource exposure patterns
- Prompt template design
- Error handling and response formatting
- Integration with shared libraries
- Async patterns for MCP protocol"
    fi
...
- GCP service account credential handling
- Domain-wide delegation security
- Input validation for customer IDs
...
- Google API rate limiting and batching
...
- Real GCP integration tests vs mocks
- Test fixture cleanup (especially for Drive/Sheets/Slides files)
- Rate limit handling in tests (delays between tests)
- Domain-wide delegation in test credentials
...
- Ruff formatting compliance (100 char line length)
- Type hints required on all functions
- Pydantic for data models
- Namespace package import patterns (from growthnav.bigquery import ...)
...
### Repository Context
GrowthNav is a uv workspace monorepo providing shared infrastructure for growth marketing analytics:
- **packages/shared-bigquery**: growthnav.bigquery - Multi-tenant BigQuery client with customer isolation
- **packages/shared-reporting**: growthnav.reporting - PDF, Google Sheets, Google Slides generation
- **packages/shared-conversions**: growthnav.conversions - Unified conversion tracking and attribution
- **packages/mcp-server**: growthnav_mcp - Unified MCP server exposing all capabilities
- **Standards**: Python 3.11+, ruff formatting (100 char), type hints required, Pydantic models

### Key Patterns
- Namespace packages: \`from growthnav.bigquery import TenantBigQueryClient\`
- Customer isolation: datasets named \`growthnav_{customer_id}\`
- Domain-wide delegation for Google Workspace APIs
- Real GCP integration tests (not mocks)
...
---
*Review generated by Claude Local PR Review Tool for GrowthNav*
EOF
...
case "$OUTPUT_MODE" in
    "comment"|"draft-comment")
        ...
        echo "• Run: uv run ruff check packages/ && uv run pytest packages/"
...
echo ""
echo -e "${BLUE}Claude Review Script for GrowthNav v1.0${NC}"
